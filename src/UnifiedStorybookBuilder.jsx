...
export default function UnifiedStorybookBuilder() {
  ...
  const pages = useMemo(
    () =>
      buildStoryPages(story, {
        name: name || (archetype === "BoyBase" ? "Buddy" : "Sunny"),
        hair: hairColor,
        favoriteColor: accentColor,
        favoriteAnimal: favoriteAnimal || "bunny",
        homeTown: homeTown || "the park",
      }),
    [story, name, archetype, hairColor, accentColor, favoriteAnimal, homeTown]
  );
  ...
  async function handleRender(pageIdx) {
    const body = {
      ...
      story,
      pageIndex: pageIdx,
      sceneText: pages[pageIdx].text,
      renderPlan,
      promptTemplate,
    };
    try {
      const r = await fetch("/render-page", {
        method: "POST",
  ...
  const apiPayload = useMemo(
    () => ({
      ...
      workflow: {
        ...
        styleLoRA: characterSpec.style.lora,
        characterLoRA: archetype === "BoyBase" ? "lora://kid_boy_base" : "lora://kid_girl_base",
        controlNet: "openpose",
      },
    }),
    [characterSpec, story, renderPlan, promptTemplate, pages, archetype]
  );
        ...
        <TabsContent value="export">
          <Card className="shadow-md">
            ...
            <CardContent className="space-y-3">
              ...
              <pre className="text-xs bg-neutral-900 text-white p-3 rounded-lg overflow-auto max-h-[340px]">{json}</pre>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
